...
# Block 0 checks to see if this is the first time running the script and exits until UFC.py creates the correct directories
...
UFCmover_running_holder_list = glob.glob(info_check.mma_direct+'mover-running.holder') # check to see if this the "mover-running.holder" file exists (if it does, that means this UFCmover.py script is currently running)
if len(UFCmover_running_holder_list) == 1:
    log = open(info_check.ufc_direct+'stop-log.txt','a')
    log.write(addts+"An attempt to run UFCmover.py was made. However, UFCmover.py is currently running because mover-running.holder is present.")
    log.close()
    exit() # end the script if the "mover-running.holder" is present
else:
    UFCmover_running_holder = open(info_check.mma_direct+'mover-running.holder','w') # if "mover-running.holder" doesn't exist (the script is NOT currently running), then create "mover-running.holder"
    UFCmover_running_holder.write(ts+" UFCmover.py started running.")
    UFCmover_running_holder.close()
    logger("UFCmover.py started running.")
# 1111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111
...
UFC_running_holder_list = glob.glob(info_check.ufc_direct+'UFCrunning.holder') # check to see if the "UFCrunning.holder" file exists (if it does, that means this UFC.py script is currently running)
if len(UFC_running_holder_list) < 1:
    with open(info_check.ufc_direct+'log.txt','r') as myfile:
    ...
    if (second_dif > 1814400) and (len(UFC_running_holder_list) < 1): # if the current log.txt file is has over 3 weeks of logs and the other script is not running then proceed
        previous_log_holder_list = glob.glob(info_check.ufc_direct+'previous-log.txt') # check to see if the "previous-log.txt" file exists
        if len(previous_log_holder_list) == 1:
            os.remove(info_check.ufc_direct+'previous-log.txt')
        move(info_check.ufc_direct+'log.txt', info_check.ufc_direct+'previous-log.txt')
        filename = info_check.ufc_direct+"log.txt"
...
for root, dirnames, filenames in os.walk(user_info.ufc_destination): # this directory is where all new events directories will be created
...
for root, dirnames, filenames in os.walk(user_info.ufc_destination): # this directory is where all new events directories will be created
    for filename in fnmatch.filter(filenames, '*Soon - Early Prelims.avi'): # look for all 'holder' files that were created with each new event directory
        video_holder_path_and_file.append(os.path.join(root,filename))
        video_holder_path.append(root)
...
for root, dirnames, filenames in os.walk(user_info.ufc_destination): # this directory is where all new events directories will be created
    for filename in fnmatch.filter(filenames, '*Soon - Prelims.avi'): # look for all 'holder' files that were created with each new event directory
        video_holder_path_and_file.append(os.path.join(root,filename))
        video_holder_path.append(root)
...
for x in range(0,len(video_holder_filename)):
    ...
    for y in range(0,len(completed_video_path_and_filename)):
        ...
        completed_video_name_no_leading_s = re.sub(r's(?=[0-9][0-9])','',completed_video_name_no_spaces)
                ...
                if 'mkv' in video_name_search_terms:
                    ...
                    for node in os.listdir(user_info.tmp_dir):
                        if not os.path.isdir(node):
                            move(os.path.join(user_info.tmp_dir, node) , os.path.join(user_info.ufc_destination, node))
                            urllib.request.urlopen('http://localhost:32400/library/sections/'+plex_token.section+'/refresh?X-Plex-Token='+plex_token.token)
                    logger("Directories and files moved back to"+buf+user_info.ufc_destination+buf+"in order to force pleX to refresh. The script will stop running now.")
                    endit()
                else:
